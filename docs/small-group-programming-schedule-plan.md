...
### 2.13 入会オンボーディングフロー（入会処理→担当講師決定）— 2026-07-13 設計

**ゴール**: 入会処理から担当決定までを1つの流れで完結させる（部品は揃っているが横断する導線が無い）。UXの核 = 「次に何をすべきか迷わない」「空き状況を見ながら決められる」。

**入口**:
1. 問合せ詳細「生徒として登録」→ 生徒作成後、従来の `/students?edit=` ではなく **`/students/[id]/onboarding`（ウィザード）へ遷移**
2. 生徒詳細・一覧に「通塾セットアップ」導線（通塾日程が0件の生徒に表示。問合せ経由でない直接入会にも対応）

**ウィザード構成**（既存のステップUIパターン踏襲: paste取込/zoukomaフォームと同型）:
- **Step1 生徒情報の確認**: 問合せからの転記内容（氏名/かな/学年/在籍校）を確認・修正
- **Step2 受講科目と比率**: 学年区分で絞った科目を複数選択し、科目ごとに 1対2（既定）/1対1 を選ぶ（= student_subject_contracts の入力。45分科目は表示で明示）
- **Step3 通塾日程**: **週間ミニグリッド**（列=曜日、行=個別コマ時間）。セルに空き状況（出勤講師の空き席数）を表示し、クリックで「そのコマに入れる科目」を選ぶ。複数セル選択=週複数回。45分は前半/後半を選択
- **Step4 担当講師**: 選んだ各コマにマッチング候補（既存スコア: 担当固定+50/過去+30/教科/性別/出勤）を上位表示 → クリックで決定 or「未決定のまま」（後で一括マッチングに乗る）
- **Step5 確認・一括保存**: 契約 upsert×科目 + createRegularPattern×コマ（ratio/duration/half込み）+ 対象週の再生成 → 完了画面から「座席表を見る」

**体験コマの引き継ぎ（欠落の是正）**: 問合せ経由の入会時、`schedule_entries.inquiry_id` の体験コマを `student_id` へ付け替え（XOR整合。同一枠の重複はスキップして報告）。体験履歴が生徒の実績として残る

**スコープ外**: inquiries.phone/email の生徒転記（students に列が無い。別案件）/ マッチング画面への生徒フィルタ追加（Step4が代替）

### 2.13改訂（2026-07-13 実機フィードバック）: Step3とStep4を統合し「実データのミニ座席表にD&D」

初版の Step3（人数表示グリッド）＋Step4（別画面のマッチング候補）を**1画面に統合**。担当決定は「実際の座席表を見ながら、出勤している講師に生徒をD&Dで置く」形にする。1対2は隣の生徒との相性・講師との相性が繊細なので、その場で見えることが重要。

**新・Step3「コマ配置（ミニ座席表）」**（ウィザードは5→4ステップ: 1生徒情報 / 2受講科目 / 3配置 / 4確認）:
- **配置する生徒カード（ドラッグ元）**: Step2 の受講科目から「今置く科目」を選ぶ小カード（例「田中 数学」）。ドラッグ可能。45分科目は前半/後半の別カード
- **週間グリッド overview**: 列=曜日、行=個別コマ時間。各セルに、その曜日×コマに**出勤できる講師を実データで小カード表示**（getAvailabilityDayMap の出勤講師＋getScheduleEntries の現在の座席）。数字（人数）は廃止
- **ミニ座席表**: セルをクリック（またはドラッグ開始）でそのコマの**ミニ座席表**を展開＝各講師カードに現在の在籍生徒（1対2の隣の生徒が見える）＋空席。座席表本体の TeacherCard/StudentCard の見た目言語を流用
- **D&D で配置**: 生徒カードを講師カード（または空席）へドロップ→配置確定。可否・相性は既存 `evaluateStudentDrop`（満席/1対1/指導科目外/性別/除外）を流用してドロップ可否＋理由表示。1対2で隣に既存生徒がいる状態が見える
- **配置の累積**: 置いた分はミニ座席表に即反映（次の配置で隣として見える）。週N コマのカウント表示
- **担当は暗黙決定**: どの講師カードに落としたか＝担当講師。別画面のマッチングは廃止。相性ヒント（指導可否・性別一致）は講師カードのバッジで示す（Step4の候補スコアの代替）
- **期間設定**: 「通塾開始日」の日付ピッカー（既定=今日。クイック選択: 今日 / 来月1日）。中途半端な開始（来月から等）に対応。バッチ全体の effective_from に反映
- **スキップ**: 「通塾設定をスキップ」ボタン→生徒登録だけで完了（パターン未作成。後でセットアップ）

**Step4 確認・保存**: 配置一覧（曜日×コマ×講師×科目×比率）＋開始日を確認→一括保存（契約 upsert / createRegularPattern with effective_from=開始日 / 開始日の週を再生成 / 体験コマ引き継ぎ）。

**表示する週**: 開始日を含む週の座席表を土台に表示（開始時点の空き状況で判断できる）。

### 2.13改訂2（2026-07-13 実機フィードバック）: 役割分担の明確化（いつ/何→Step2、誰→Step3）

操作に迷わないよう、Step2 で「科目・曜日・時限・比率」まで決め切り、Step3 は「その各コマの担当講師をミニ座席表で決める」だけにする。

- **Step2「受講科目」（拡張）**: 学年で絞った科目を複数選択。**選択した各科目に 曜日 と 時限（コマ）を選ばせる**（＋比率 1:2/1:1、45分なら前半/後半）。ここで「何を・いつ・何対何で」が確定
- **Step3「コマ配置」（簡素化）**:
  - **① 通塾開始日**（見出しに番号①）: 日付ピッカー（今日 / 来月1日）
  - **② スケジュール**（見出しに番号②）: Step2 で決めた (科目×曜日×コマ) だけを**その曜日・コマのミニ座席表として詳しく表示**（全6×5グリッドは出さない）。担当決定は D&D:
    - **ドラッグできるカードを受講科目の数だけ並べる**（1科目=1カード）。**科目名の色バッジは付けない**（カード自体が科目。カードには科目名＋生徒名＋比率）。従来の科目トグルチップ（国語/理科の切替）は廃止
    - 各カードを、対応するコマのミニ座席表の講師へ D&D →担当確定。1対2の隣の生徒が見える。可否・相性は evaluateStudentDrop 流用
- 配置は「どの講師に落としたか」＝担当。Step2 で曜日・コマは確定済みなので Step3 は講師選びに集中（迷わない）

### 2.14 生徒の講師希望（NG講師・固定講師・希望性別）の設定UI — 2026-07-14

`students.excluded_teacher_ids`（担当NG）・`fixed_teacher_ids`（固定）・`preferred_teacher_gender`（希望性別）は Phase 0 で列追加済みで、D&D の可否判定・マッチング候補スコア・落とし込みの理由表示が**既にこれらを読んでいる**が、**設定するUIが存在しない**ため常に空＝制約が発動しない。設定UIを追加する。

- 生徒編集（StudentForm / 生徒詳細）に「担当講師の希望」セクション: **担当NG講師**（教室の講師を複数選択）／**固定講師**（複数選択）／**希望性別**（指定なし/男性/女性）
- 講師リストは admin users API（role=teacher、座席表と同じ取得）。updateStudent がこれら3列を保存するよう API/型を確認・配線
- 保存後、座席表のD&D・落とし込み・入会ウィザードで NG講師には落とせず理由（担当除外指定）が出る＝既存ロジックがそのまま効く
- ヘルプFAQ更新
