...
> **⚠️ 2026-07-06 追記（本文より優先して読むこと）**
>
> 1. **本番DBは移行済み**: 本書が各所で参照する `mzxysqkuuxcfffwlfsvj`（シンガポール）は**旧DB**。2026-06-23 に東京リージョン `bniistrbylypnwpfqszb` へ移行済み（`.env.local` の `NEXT_PUBLIC_SUPABASE_URL` 参照）。以降の SQL/マイグレは新DBに対して行う。
> 2. **報告書×進行表マージの前提訂正**: §6 残課題で「`progress_sessions.report_id` で紐付け基盤あり」としているが、**そのカラムは存在しない**（型・マイグレとも無し。`report_id` は `lesson_report_units` の別FKのみ）。マージ本実装はスキーマ設計（紐付けカラム追加）から着手が必要。現状は `DemoProgressPreview` のダミー止まり。
> 3. スケジュール機能自体は本書記載範囲（H〜O＋講習本実装）で安定稼働。6月以降の変更は運用改善が中心で、大きな構造変更なし。ロードマップとの突合は `docs/release-roadmap-2026H2.md` の「棚卸メモ（2026-07-06）」を参照。
...
- **データソース**: `form_responses`(form*type='zoukoma', `linked_student_id` 紐付け済み)。専用テーブルは作らない。科目はフォームの**名前キー**→Subjマスタへ**名前一致**。通塾枠は `response_data.selected_slots`（"YYYY-MM-DD*時限コード"）。時限の開始時刻は `form_periods.settings.schedule.periods`。
...
### UX整理（2026-06-02）— ツールバー集約＋テスト対策の期間非依存化

- **モード1本化**：講習/テスト対策の2セレクト→「モード」1セレクト（`<optgroup>` で 通常/講習(期間)/テスト対策）。`ScheduleToolbar` は `selectedKoushu`/`hasTestPrep`/`testPrepActive` ＋ `onKoushuSelect`/`onTestPrepToggle` を受け、内部で `modeValue`('' / `k:<id>` / `t`) を組む。
...
### 出席可能日程ストリップ（2026-06-12）

配置モード（講習/テスト対策）で「配置」を押すと、配置パネルと座席表の間に**期間全日×時限のドットマトリクス**を表示（モック=`docs/mockups/placement-availability-strip.html` 案E が最終形。A〜D は検討過程）。
...
2. **報告書 × 進行表のマージ本実装**：現状はダミーイメージ (`DemoProgressPreview`) のみ。合意済みゴール = 「**入力1つ・ビュー2つ**（講師/室長向け進行表ビュー + 保護者向け報告書ビュー）」。重複項目（学校進度・単元・講師コメント）の一本化が前提。**⚠️ 2026-07-06 訂正: 「`progress_sessions.report_id` で紐付け基盤あり」は誤り**。該当カラムは存在しない（`report_id` は `lesson_report_units` の別FKのみ）。**紐付けスキーマの新設から**必要
...
## 指導形態ボード（Phase C+D / 2026-07-10）

小集団・プログラミング等の「指導形態ごとのボード（タブ切替）」を実装。正典計画は `docs/small-group-programming-schedule-plan.md`。

### 全体像

- `/schedule` 上部に**形態タブ**（個別＋ is_active なユーザー定義形態、`getFormations()` の sort_order 順）。ユーザー定義形態が0件ならタブバーは出ない（＝従来の見た目）。
- **個別タブ = 従来画面そのまま**（講習/テスト対策/マッチング/D&D/印刷すべて不変）。
- **形態タブ = 形態ボード**（`FormationBoard.tsx`）。その形態のコマ時間×曜日グリッド。向きトグル（日=行/日=列）に追随。1セル = クラス枠カード群（1講師＝1カード、講師名＋生徒N行＋定員 `count/max`）。空席は緑破線プレースホルダ（最大2行＋「ほか残N席」で圧縮）。

### 登録・データフロー（Phase C）

- セルの「＋クラス枠」→ `FormationKomaFormModal`（曜日×コマ自動設定・講師1名/担当未決定可・生徒複数・科目任意）。
- 保存は `createFormationClassPatterns`（`src/lib/api/formation-patterns.ts`）＝生徒ごとに `schedule_regular_patterns`（formation=キー, effective_from=今日, period_type='regular'）を作成。既存 `createRegularPattern` を再利用。
- **バリデーション**（DB書き込み前にまとめて検査）:
  1. 定員: 同一(曜日×コマ×講師)の既存メンバー + 追加数 ≤ `school_formation_capacity.max_students_per_group`（未設定=8）
  2. 同時刻枠数: 新規講師枠を増やすときのみ、同一(曜日×コマ)の講師枠数 < `max_concurrent_groups`（未設定=1）
  3. 講師の別枠時間帯重複: 同一講師が**別コマ**（時間帯オーバーラップ）にいれば不可。同一コマは複数生徒OK
  4. 生徒: `checkStudentTimeConflict`（形態横断・同時間帯の二重登録禁止）
- 保存後 `regenerateCurrentWeekIfNeeded` + `refreshEntries` で座席表へ反映。`generateWeeklySchedule` は formation を継承済みなので形態エントリも生成される（週全DELETE→再INSERT の transferredKeys 保護は不変）。
- 空席行クリック = 既存クラスへの生徒追加（同モーダルの add モード・講師固定）。

### 操作・振替

- 生徒行クリック → 既存 `StudentActionModal`（欠席/振替/編集/削除）を流用。削除（regular）は既存フローで通塾日程から外れる。
- **振替**: 形態タブではグリッドの講師ブロッククリック方式が使えないため、`handleTransferFromAction` を分岐させ**モーダル方式**（`TransferModal`）に切替。`ScheduleDialogs` へ渡す `timeSlots` を**その形態のコマに限定**しているので、振替先コマはその形態内に限られる。
- `createTransferEntry` に **formation 継承**を追加（振替先が個別ボードへ化けるのを防ぐ）。

### 一覧（Phase C-4）

- `/schedule/regular-patterns` に**形態フィルタ**（select・既定=全形態）＋**形態バッジ列**を追加（`RegularPatternTable` の `formationLabels` prop）。

### 形態タブで隠す個別専用UI

モード select（講習/テスト対策）・列数トグル・未配置バナー・ドリフトバナー・講習/テスト対策パネル・配置ストリップ・未消化振替ボードは形態タブで非表示。向き/表示曜日/座席表の設定/管理▾は共通で残す。

### 迷った判断（要確認）

- **科目 = 任意の複数選択**（select ではなくチップ複数可）にした。プログラミングのコース名は科目マスタ管理しない前提。
- **生徒の削除**は effective_until=昨日ではなく既存の削除フロー（cancelFutureEntries + is_active=false）を流用。形態クラスは請求連動スコープ外のため実害なし（`removeStudentFromFormationClass` は用意済みだが未配線）。
- **クラス見出し** = クラス名概念が無いので科目名の和集合（無ければ「クラス」）を表示。定員チップは残席ありで緑。

### DBマイグレーション

不要（Phase A の `schedule_formations` / `school_formation_capacity` で足りる）。

---

_最終更新: 2026-07-10 / 指導形態ボード（Phase C+D）完了。個別タブは不変、形態タブで小集団・プログラミング等のクラス枠を運用可能に_
_（旧）2026-06-02 / 講習（季節講習）モード本実装（P/U/K/M/N ラウンド）+ 担当割当の耐性化 + course_prep RLS 是正 完了時点_
