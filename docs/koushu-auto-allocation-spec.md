...
| 24  | 教室の代行入力                         | **作る**。ただし出所（保護者本人／教室代行）と入力者IDを記録し、一覧にバッジ・集計で別掲・`admin_audit_logs` に記録（§14-2）  |
...
スマホを使わない家庭のために代行入力は**作る**。ただし「教室長が自分で申し込んで、取ったように見せる」
を抑止するため、代行だと消えないように残す。

| 仕組み       | 内容                                                                             |
| ------------ | -------------------------------------------------------------------------------- |
| 出所の記録   | 申込に `source: 'parent' \| 'staff'` と入力者（`created_by`）・日時を持つ        |
| 一覧のバッジ | 提案書一覧・申込状況に「代行」バッジを出す                                       |
| 集計の別掲   | 達成率・取得コマの集計で代行分を別に数えられるようにする（本部が見て区別できる） |
| 監査ログ     | 代行入力を `admin_audit_logs` に記録する                                         |

**保護者への自動確認メールは送らない**（検討したが採用せず）。

実装メモ: `koushu_enrollments` に `source` / `created_by` 列を追加する。既存行は本番0件なので移行不要。
`admin_audit_logs` は manager+ に絞られている（`project_rls_security_remediation`）ので参照側の権限に注意。
...
- 通常授業コマ数の求め方は `schedule_entries`(kind='regular') の期間内件数を科目ごとに数える
  （週回数×週数の概算ではなく実データ。休講・振替を正しく反映するため）
