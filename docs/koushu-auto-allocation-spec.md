...
## 20. Q5 本番配線の実装（2026-08-08 完了）

DDLなし。旧 `generateKoushuIndividualProposals` を削除し、アロケータ（第1部 §5）を
本番の下書き生成に接続した。
...
| 層                          | 役割                                                       |
| --------------------------- | ---------------------------------------------------------- |
| `realDataAdapter.ts`        | DBを読んで `AllocatorInput` を組む（**読み取り専用**）     |
| `allocate.ts`               | 純粋関数の配置ロジック（DB非依存・テスト可能）             |
| `koushu-match.ts`           | 上2つをつなぎ、結果を下書き提案として保存                  |
| `KoushuAllocationPanel.tsx` | 実行パネル（学年選択・実行時設定・再実行モード・結果表示） |

### 20-2. 旧実装を残さずに消した理由

旧 `generateKoushuIndividualProposals` は次の点で**間違ったデータを読んでいた**。

- 生徒の通塾可能表が**未配線**（`settingId` にコメントで「現状は未配線」とあった）＝
  通えない日にも配置していた
- 講師の出勤が通年の曜日別出勤可能（`getAvailabilityDayMap`）で、講習シフト提出を見ていない
- 席の判定が単純な人数比較で、1対1の排他・45分の前半/後半を数えられていない

同じ画面から呼べる状態で残すと古い経路が生き続けるため、**フォールバックは用意しない**。
`scoreTeacher` / `enumeratePeriodDates` とそのテスト16件も削除した
（指名NG・希望性別のハード除外は `koushuAllocator.test.ts` の不変条件検証が
毎回の結果に対して確かめているので、業務ルールの検証は失われていない）。

### 20-3. 再実行モード（§5-5 の実装）

| モード   | 既存下書き                                       | batches.mode |
| -------- | ------------------------------------------------ | ------------ |
| **破棄** | 実行前に `status='dismissed'` にしてから組み直す | `overwrite`  |
| **差分** | 占有として積み、埋まっていないコマだけ足す       | `diff`       |

- どちらも**公開済みエントリと手動配置には触れない**
- 破棄は物理削除しない（何を捨てたか追える）
- 破棄を「実行前」にやるのが要。後にすると、下書きを積まずに組んだ提案と古い下書きが
  座席表に二重で★表示される

### 20-4. 未割当の理由（§5-3 の5分類）を画面へ

`groupUnassignedByReason`（純関数・テスト8件）で理由別に合算し、コマ数の多い理由から出す
＝対処の優先順になる。理由と対処の対応:
...
### 20-5. 実データの欠損を黙って劣化させない

実行結果に `notes` を出す。次のときは画面に注意書きが出る。

- **Web申込が0件**（紙運用と並走中）→ `student_progress` から科目を推定。
  「科目は暫定割当」と明示する
- 講習の可能表が未提出 → 普段の通塾日程を可能枠として使った旨
- 講習シフトの提出者をアカウントに紐付けられなかった → 名前を挙げて「出勤として数えていない」
- コマ時間が解決できなかったシフト行 → 件数（過去期間とコマ時間が変わっている可能性）

### 20-6. 残っている作業

- **実機確認が未了**（要ログイン）。特に「実行 → 座席表に★が出る → 公開」の一連
- Q6（2027年2月の切替）。紙運用の停止と、申込の正典を Web申込に一本化する判断
