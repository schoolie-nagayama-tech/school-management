...
| 25  | 提案外の科目                           | **保護者が追加できる**。形式は既定（1対2・90分）で受け、教室が後から調整する（§14-3）                                         |
...
提案書に無い科目もやりたい場合があるため、フォームから科目を追加できる（決定25）。

- 選べる科目は `subjects` のうち生徒の `grade_category` に合う行
- 追加分は**教材が未定**。形式は既定（1対2・90分）で受け、画面には「形式は教室で調整します」と出す
- 教室が後から形式を変えると金額が変わる。**申込時の単価はスナップショットする**（§15）ので、
  変更する場合は教室から保護者へ連絡する運用とする
