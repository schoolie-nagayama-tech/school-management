...
| 科目の均等分散 | ON | 同一科目の本数を期間全体に等間隔で散らす |
...
生徒を「可能枠セル数の少ない順 → 残本数の多い順」でソート。生徒内では科目をラウンドロビン（1本ずつ交互）で消化し、特定科目が枠を食い尽くすのを防ぐ。
...
- 科目の均等分散: 同一科目の既配置との日間隔が理想間隔（期間日数÷本数）に近いほど加点／同日・隣日に固まると減点（設定ONのとき）
