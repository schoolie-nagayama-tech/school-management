...
作成: 2026-07-16 / **最終更新: 2026-07-20（成績デモデータ本番投入済み・成績への最小導線を追加）**

**目的**: 保護者ポータルv2を本番に「ダミーデータだけのデモ」として置き、**admin（4名）だけ**に触ってもらう。実データには一切つなげない。
**仕様の正典**: `docs/portal-v2-requirements.md`（デモの設計は **§6-5**、成績は **§7-5**）

---

## 0. 次に何をするか（2026-07-20 時点・引継ぎの起点）

**本番DBには成績デモデータまで投入済み。コード側（成績への最小導線＋demo SQL §14）はブランチ `claude/guardian-portal-revision-39ae63` のPRでマージ待ち**（マージされたらこの一文を消すこと）。本番DBは最新（最終マイグレーション `20260717051921` = portal_v2_scores。以降はデータのみで DDL 変更なし）。

| # | 残タスク | 状態・メモ |
|---|---|---|
| 1 | **admin の実機確認**（唯一の必須の宿題） | 本番で 歯車→「保護者ポータルV2（試作・ダミーデータ）」→ デモが開くか。**`PORTAL_JWT_PRIVATE_JWK`（Vercel env）の実地検証を兼ねる**。失敗したら env を疑う。外形（未ログイン404・API 401）は確認済み。成績は `/mypage/grades`（導線PRマージ前は直URL）で承認待ち/差し戻し/承認済みが見えるはず |
| 2 | 成績（Stage5）の導線 | **最小導線のみ設置**（2026-07-20・上記PR）: ダッシュボード末尾に `/mypage/grades` への静かなリンク行1つ（`DashboardView.tsx` の `GradesLink`）。**カード化はしていない**（簡素化直後のため、デモの感触を見てから判断） |
| 3 | ~~成績のデモデータが未投入~~ → **✅ 本番投入済み（2026-07-20）** | demo SQL に §14 を追加し全体を本番へ再実行。実測: 申請4件（承認待ち1・承認済み2・差し戻し1）／デモ生徒の `assessments` 4件（スタッフ直接入力の模試・小6内申を含む）／科目点24件／保護者視点（portal ロール実測）で成績・申請とも4件可視。**報告書の `subject_specific` 未反映も同時に解消**（§5 の⚠） |
| 4 | 本番 `/`・`/login` の匿名500 | **本件の回帰ではない既存事象**（旧ビルドから発生）。実ブラウザでは正常描画。別タスクで調査中 |
| 5 | ダッシュボードの追い込み | 2026-07-17 に大幅簡素化してマージ済み。ユーザーがスマホで再確認して、まだ多い/削りすぎがあれば次の一巡 |
| 6 | help の FAQ_DATA 更新（§6-2） | Stage4/5・デモ・成績とも未更新のまま（運用ルール上の宿題） |

**成績デモの一周の仕方（admin向け）**: `/mypage/grades` で承認待ち「1学期期末」を確認 → スタッフ側 `/students/[体験太郎]/scores` の承認待ちバナーから承認 → 保護者側で承認済みに変わり一覧に載る。差し戻しの文面例は花子の申請（学年間違い）。**再実行すれば必ず初期状態（承認待ち1件）に戻る**（§14 は delete→再生成。承認で生まれた行も消える）。

**ローカル環境の注意**: Docker Desktop はエージェントから起動できない（ユーザーが手で起動）。ローカルSupabaseは `npx supabase start`。**staff@test.local のパスワードは不明**（`tybiz1452@gmail.com` / `password123` は通る）。
...
| | 状態 |
|---|---|
| コード | **Stage1〜5 すべて main にマージ済み**。＋成績の最小導線とdemo SQL §14 が `claude/guardian-portal-revision-39ae63` のPRで**マージ待ち**（2026-07-20） |
| 本番DB | **マイグレーション6本＋デモデータ適用済み**（5本=2026-07-16／`portal_v2_scores`=2026-07-17／**成績デモデータ＋subject_specific=2026-07-20**） |
| フラグ | `portal_v2_enabled` = **false のまま**（＝一般公開されていない。ONにしないこと） |
| 公開範囲 | **admin 4名のみ**（歯車メニュー・デモ校の user_schools とも） |
| Vercel env | `PORTAL_JWT_PRIVATE_JWK` は**ユーザーが設定済み**（2026-07-16 申告）。**未検証**（上記の残タスク1） |
| push / merge | ✅ PR #9（本体）/ #11（掲示板配信先を非表示）/ #13（CI赤の修正）/ #14（成績）/ #16（ダッシュボード簡素化）すべてマージ・main CI green |

### この日（2026-07-17）にやったこと
- **Stage5 成績を実装・マージ（PR #14）** — 保護者入力→スタッフ承認→`assessments` へ転記。本番マイグレーション適用済み・権限も本番実測（anon=ゼロ / authenticated=SELECTのみ / portal=SELECTのみ）。**ローカル実機で一周確認済み**（入力→承認→既存の成績画面に反映→保護者側にも承認済みとして表示）
- **掲示板の配信先UIを非表示化（PR #11）** — `AUDIENCE_UI_ENABLED=false`。新規投稿は常に社内。**編集時は audience 系キーを送らない**＝デモ校の「保護者向けお知らせ」が編集で巻き戻らない
- **ダッシュボードを大幅簡素化（PR #16）** — +48/−200行。講師名・続く予定2行・講評抜粋・プレビュー文・「ほかにN件」・「受付中」項目を削除。各カード「大きな1要素＋最小限」に
- **main の CI 赤を修正（PR #13）** — ★教訓: `gh pr merge --auto` は必須チェック未設定のリポジトリでは事実上の即時マージ。**マージ後は必ず `gh run list --branch main` で結論を確認する**（PR #11 で CI 赤のまま main に入れてしまった）
...
| 誰に | 何が見える | 挙動 |
|---|---|---|
| 全スタッフ | ~~掲示板の投稿モーダルに「配信先」欄~~ | **PR #11 で非表示に戻した（2026-07-17）**。従来どおりの見た目・常に社内配信 |
...
| **成績を編集できる人（講師含む）** | `/students/[id]/scores` の上部に**承認待ちバナー**（Stage5・2026-07-17） | **その生徒に承認待ちの申請があるときだけ**出る。本番は申請0件なので**今は誰にも出ない** |
...
### 3-6. Stage5（成績）の本番適用 — ✅ **2026-07-17 完了**
`20260717000000_portal_v2_scores.sql` を MCP の `apply_migration` で適用（本番版名 `portal_v2_scores` / 版番号 `20260717051921`）。**版番号ズレがこれで6本目**（§3-2 の表に追加。`db push` 禁止は継続）。

適用後の本番実測:
- テーブル `portal_score_submissions` / ビュー `portal_assessments` / 索引5 / ポリシー2 すべて存在
- **権限が設計どおり**: `anon`=**ゼロ** / `authenticated`=**SELECTのみ** / `portal`=**SELECTのみ**
- 制約スモーク（模試拒否・同一枠の承認待ち重複拒否・差し戻し理由必須）を `BEGIN…ROLLBACK` で実測し**残留ゼロ**

**★ Supabase の既定権限の罠（この適用で発覚・重要）**: `public` の新規テーブル/ビューには `alter default privileges` により **anon と authenticated に ALL が自動付与される**。`grant select ... to authenticated` と書いても「SELECTのみ」にはならない。実際に止めているのは RLS（ビューは述語）であって GRANT ではない。**既存の `portal_report_reads` / `portal_class_reports` も同じ状態**（プロジェクト全体の既定パターン。新たな脆弱性ではない）。成績は機微なので Stage5 だけ `revoke all from anon, authenticated` してから付け直した。→ 新しい portal 系オブジェクトを作るときは同じ手当てをするか、コメントに「SELECTのみ」と**書かない**こと。

**成績のデータ状況（本番）**: ~~0件~~ → **✅ 2026-07-20 投入済み**（demo SQL §14）。`portal_score_submissions`=4件（承認待ち1・承認済み2・差し戻し1）／デモ生徒の `assessments`=4件・科目点24件。冪等（delete→再生成方式。デモ中に承認して増えた行も再実行で消え、常に「承認待ち1件」の初期状態に戻る）。
...
**デモSQLと本番の同期状況**: ~~2026-07-16 投入後の `subject_specific` 追加が本番未反映~~ → **✅ 2026-07-20 に成績（§14）追加と合わせて全体を本番へ再実行し解消**（実測: 太郎の報告書2本に `subject_specific` あり・花子は null が正）。以後も demo SQL を更新したら「本番へ全体を再実行」が反映手段（冪等）。実行手段は §3-3 のメモ参照（MCP `execute_sql`・スクラッチ照合つき）。
