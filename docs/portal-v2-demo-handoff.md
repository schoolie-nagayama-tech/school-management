...
作成: 2026-07-16 / **最終更新: 2026-07-17（Stage5 成績まで本番反映済み）**
...
## 0. 次に何をするか（2026-07-17 時点・引継ぎの起点）

**未マージのPRは無い。作業ツリーはクリーン。main の CI は green（797a2ec）。本番DBも最新（最終マイグレーション `20260717051921` = portal_v2_scores）。**

| # | 残タスク | 状態・メモ |
|---|---|---|
| 1 | **admin の実機確認**（唯一の必須の宿題） | 本番で 歯車→「保護者ポータルV2（試作・ダミーデータ）」→ デモが開くか。**`PORTAL_JWT_PRIVATE_JWK`（Vercel env）の実地検証を兼ねる**。失敗したら env を疑う。外形（未ログイン404・API 401）は確認済み |
| 2 | **成績（Stage5）の導線が未設置** | `/mypage/grades` は動くが、**ダッシュボード・メニューからのリンクが無い**（直URLでのみ到達）。ダッシュボードを簡素化した直後なので、何をどう足すかはデモの感触で決める方針にした |
| 3 | **成績のデモデータが未投入** | 承認待ち1件＋差し戻し1件＋承認済み数件を demo SQL に足す想定（§7-5「デモ・検証」）。本番の `portal_score_submissions` は**0件**、デモ生徒の `assessments` も**0件**＝成績カードは空の状態 |
...
| | 状態 |
|---|---|
| コード | **Stage1〜5 すべて main にマージ済み**。未マージPRなし・作業ツリークリーン |
| 本番DB | **マイグレーション6本＋デモデータ適用済み**（5本=2026-07-16／`portal_v2_scores`=2026-07-17） |
...
**成績のデータ状況（本番）**: `portal_score_submissions`=**0件** / デモ生徒の `assessments`=**0件**＝ デモの成績カードは空。§7-5「デモ・検証」のサンプル（承認待ち1・差し戻し1・承認済み数件）は**未投入**。
