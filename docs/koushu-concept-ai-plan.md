...
## 2. 置く場所 ＝ 既存の「講習テーマ」欄（2026-09-03 決定）

**★`seasonal_proposals.theme` をそのまま使う。列は足さない。**（ユーザー: 「コンセプト書くとこあるよね。そこを使おう」）

- 入力欄を1行から複数行にする（列は text なので変更不要）。
- 印刷（`ProposalPrintView.tsx:84`）と申込フォーム（`ApplyProposalLine.theme`）では段落として出す。
- 一覧（`ProposalList.tsx:589`）はテーマをタイトル代わりに出しているので、**先頭1行だけ**を出す。
- 「1学期の文法を組み直す」のような短い一言はそのまま**1行目**に書けばよい。コンセプトはその続き。

以下は決定前の調査（列が無いと判断していた経緯）。
...
- 保護者側は、テーマ欄の内容がそのまま段落で出る（1行目が短い見出し、続きがコンセプト）。
