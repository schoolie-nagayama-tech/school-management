...
/**
 * 問合せメールテンプレートの管理UI（一覧 + 新規/編集/削除モーダル + テスト送信）。
 *
 * テンプレートページ（/admin/inquiries/templates）と追客メールページ
 * （/admin/inquiries/mail のタブ）の両方から使えるよう、AdminLayout や
 * 戻るリンク等の「枠」を持たない純粋な機能コンポーネントとして切り出している。
 *
 * 親へは onChanged で CRUD 完了を通知する（メールページがテンプレ一覧を
 * 再取得して送信フォームへ反映するために使う）。
 */
...
export function InquiryTemplateManager({ onChanged }: InquiryTemplateManagerProps) {
      ...
      <div className="flex items-center justify-between mb-4">
        <div>
          ...
          <p className="text-xs text-text-muted mt-0.5">
            自動送信日数を設定すると「送信候補」に自動で並びます。空欄なら手動送信専用です。
          </p>
        </div>
