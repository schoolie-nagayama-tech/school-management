...
export function AlertSidebar({ tasks, schools }: AlertSidebarProps) {
  ...
  return (
    <div className="space-y-3">
      ...
      {overdueTasks.length > 0 && (
        <div className="bg-red-50 border border-red-200 rounded-lg p-3">
          <div className="flex items-center gap-1.5 mb-2">
            ...
            <h3 className="text-xs font-bold text-red-700">期日超過 ({overdueTasks.length})</h3>
          </div>
      ...
      {/* 今日のタスク */}
      <div
        className={`border rounded-lg p-3 ${todayTasks.length > 0 ? 'bg-blue-50 border-blue-200' : 'bg-gray-50 border-gray-200'}`}
      >
        <div className="flex items-center gap-1.5 mb-2">
          <Clock className="w-4 h-4 text-blue-600" />
          <h3 className="text-xs font-bold text-blue-700">今日 ({todayTasks.length})</h3>
        </div>
        {todayTasks.length > 0 ? (
      ...
      {tomorrowTasks.length > 0 && (
        <div className="bg-gray-50 border border-gray-200 rounded-lg p-3">
          <div className="flex items-center gap-1.5 mb-2">
            ...
            <h3 className="text-xs font-bold text-gray-600">明日 ({tomorrowTasks.length})</h3>
          </div>
            ...
            {schoolProgress.map(({ school, completed, total, percent }) => (
              <div key={school.id}>
                <div className="flex items-center justify-between text-xs mb-0.5">
                  <span className="text-gray-600 truncate max-w-[100px]">{school.name}</span>
