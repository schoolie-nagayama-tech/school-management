...
/**
 * 家モードの講師が教室限定ページを開いたときに全画面でブロックするゲート。
 *
 * 正典: docs/teacher-home-mode-plan.md §2「家モードの強制」
 *
 * 設計:
 *   - 判定は DeviceTrustContext（役割＋端末マーク）に一本化。パスの線引きは
 *     lib/homeMode.ts の CLASSROOM_ONLY_PREFIXES に一本化。ナビの出し分けと同じ
 *     材料を見るので「メニューには無いのに開ける」ズレが生まれない。
 *   - 行き止まりにしないため、家でも使える3画面（本日の授業 / 自分の予定 / 出勤簿）
 *     への導線と、閉じ込め防止のログアウトを置く（UnreadBulletinGate と同じ作法）。
 *
 * ★ なりすまし（代理ログイン）中も出す:
 *   UnreadBulletinGate と同じ判断で、なりすまし中も講師本人と同じ体験にする
 *   （管理者が「講師には家からこう見える」を確認できる必要があるため）。
 *   ただし端末の判定は /api/device-trust/status がリクエストのクッキーで行うので、
 *   なりすまし対象ではなく **実際に操作している端末** が基準になる。自宅から
 *   なりすませば家モード、教室PCからなりすませば教室モードになる（意図どおり）。
 */
export function HomeModeGate() {
  ...
  if (!homeMode || !isClassroomOnlyPath(pathname)) return null;
