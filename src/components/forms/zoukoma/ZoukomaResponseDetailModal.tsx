    ...
    <Modal isOpen={isOpen} onClose={onClose} title="回答詳細" size="lg" minHeight="60vh">
      <div className="space-y-4">
        ...
        <div>
          ...
          <p className="text-sm text-[#4b5563]">{response.student_name}</p>
        </div>
        ...
        {/* 申込内容 */}
        <div>
          <label className="block text-sm font-medium text-[#1f2937] mb-2">科目内訳</label>
          {subjectEntries.length > 0 ? (
            <div className="space-y-1">
              {subjectEntries.map(([subject, koma]) => (
                <div
                  key={subject}
                  className="flex items-center justify-between p-2 bg-[#f9fafb] rounded text-sm"
                >
        ...
        <div className="grid grid-cols-3 gap-4">
          <div>
            <label className="block text-sm font-medium text-[#1f2937] mb-1">合計コマ数</label>
            <p className="text-sm text-[#4b5563]">{response_data.total_koma}コマ</p>
          </div>
          <div>
            <label className="block text-sm font-medium text-[#1f2937] mb-1">単価</label>
            <p className="text-sm text-[#4b5563]">¥{response_data.unit_price.toLocaleString()}</p>
          </div>
          <div>
            <label className="block text-sm font-medium text-[#1f2937] mb-1">合計金額</label>
            <p className="text-sm font-semibold text-[#1f2937]">
              ¥{response_data.total_fee.toLocaleString()}
            </p>
          </div>
        </div>

        {/* 出席できない日程（unavailable_slots がある場合） */}
        {response_data.unavailable_slots &&
          response_data.unavailable_slots.length > 0 &&
          (() => {
            ...
            const periodKeys = Array.from(new Set(allSlots.map((s) => s.id.split('_')[1]))).sort(
            ...
            return (
              <div>
                <label className="block text-sm font-medium text-[#1f2937] mb-2">
                  日程（出席不可: {response_data.unavailable_slots.length}件 / 出席可:{' '}
                  {response_data.selected_slots.length}件）
                </label>
                    ...
                    <thead>
                      <tr className="bg-[#f3f4f6]">
                        ...
                        {periodKeys.map((pk) => (
                          <th
                            key={pk}
                            className="border border-[#e5e7eb] px-3 py-2 text-center font-medium text-[#4b5563]"
                          >
                        ...
                        <tr key={dk} className={i % 2 === 0 ? 'bg-white' : 'bg-[#f9fafb]'}>
                          ...
                          {periodKeys.map((pk) => {
                            ...
                            return (
                              <td
                                key={pk}
                                className={`border border-[#e5e7eb] px-3 py-2 text-center ${isUnavailable ? 'bg-[#fef2f2]' : ''}`}
                              >
                                {!exists ? (
        ...
        {(!response_data.unavailable_slots || response_data.unavailable_slots.length === 0) &&
          response_data.selected_slots &&
          response_data.selected_slots.length > 0 &&
          (() => {
            const slotSet = new Set(response_data.selected_slots.map((s) => s.id));
            const dateKeys = Array.from(
              new Set(response_data.selected_slots.map((s) => s.id.split('_')[0]))
            ).sort();
            const periodKeys = Array.from(
              new Set(response_data.selected_slots.map((s) => s.id.split('_')[1]))
        ...
        <div className="flex justify-end pt-4 border-t border-[#e5e7eb]">
          <button
            onClick={onClose}
            className="px-4 py-2 bg-[#f3f4f6] text-[#4b5563] rounded-lg hover:bg-[#e5e7eb] active:scale-[0.97] transition-[transform,background-color] duration-150 ease-[cubic-bezier(0.23,1,0.32,1)] text-sm"
          >
            閉じる
          </button>
        </div>
