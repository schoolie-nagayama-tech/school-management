...
export function ShukaisuPeriodEditor({
  ...
  const parseWeeklyOptions = (text: string): number[] => {
    return text
      .split(',')
  ...
  const handleSave = async () => {
    ...
    try {
      const settings: ShukaisuSettings = {
      ...
      if (period) {
        ...
        if (idsToCreate && idsToCreate.length > 1) {
          ...
          await createShukaisuPeriod(createData, idsToCreate[0]);
        } else {
      ...
      <div className="space-y-6">
        ...
        <section>
          ...
          <div>
            ...
            <Input
              type="text"
              value={weeklyOptionsText}
              onChange={(e) => setWeeklyOptionsText(e.target.value)}
