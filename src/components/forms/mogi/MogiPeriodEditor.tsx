...
import {
  ...
  MOGI_EXAM_TYPE_OPTIONS_BY_REGION,
  MOGI_REGION_LABELS,
...
export function MogiPeriodEditor({
  ...
  const [region, setRegion] = useState<MogiRegion>('tokyo');
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          <label className="block text-sm font-medium text-[#1f2937] mb-2">
            地域 <span className="text-[#ef4444]">*</span>
          </label>
          <div
          ...
          >
            {(['tokyo', 'kanagawa'] as const).map((r) => (
              <button
                ...
                onClick={() => {
                  if (region === r) return;
                  // 地域変更時: 既存の日程エントリから他地域の種別をクリア
                  const allowed = new Set(MOGI_EXAM_TYPE_OPTIONS_BY_REGION[r].map((o) => o.value));
                  setDateEntries((prev) =>
                    prev.map((e) =>
                      e.examType && !allowed.has(e.examType) ? { ...e, examType: '' } : e
                    )
                  );
                  setRegion(r);
                }}
              ...
              >
                {MOGI_REGION_LABELS[r]}
              </button>
            ))}
          </div>
          <p className="text-xs text-[#4b5563]/60 mt-1">
            ※ 地域を変更すると、下の模試種別の選択肢が切り替わります。
          </p>
        </div>
                    ...
                    entry.examType === 'toritsu_v'
                      ...
                      : entry.examType === 'shiritsu_v'
                        ...
                        : entry.examType === 'jikousakusei'
                          ...
                          : entry.examType === 'zenken'
                            ? 'border-[#047857]/30 bg-[#ecfdf5]'
                            : entry.examType === 'tokushoku'
