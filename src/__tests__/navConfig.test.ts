...
describe('buildNavEntries: 講師の教室外モード', () => {
  const home = buildNavEntries(ctxFor('teacher', { outsideClassroom: true }));
  ...
  it('教室長以上には outsideClassroom を渡しても影響しない（教室外でもフル）', () => {
    const managerHome = buildNavEntries(ctxFor('manager', { outsideClassroom: false }));
