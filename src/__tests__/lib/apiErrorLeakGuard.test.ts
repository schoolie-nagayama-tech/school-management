...
const ALLOWLIST: Record<string, string> = {
  ...
  // admin 限定の自己診断ルート。返すのは Anthropic API が返した理由の原文だけで、
  // このガードが守ろうとしている「DBのカラム名・制約名が一般利用者に見える」には当たらない。
  // 原因（モデルIDの誤りなど）はその文面にしか書かれておらず、これを隠すと
  // 設定を直すためにサーバーのログを掘るしかなくなるため、この1ルートだけ許可する。
  'src/app/api/ai/help/diagnose/route.ts': 'admin限定の自己診断。外部APIの理由を出すのが目的',
};
