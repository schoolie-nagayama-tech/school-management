/**
 * コンポーネントテスト: HomeModeGate（講師の家モードのページゲート）
 *
 * 正典: docs/teacher-home-mode-plan.md §2
 *
 * 出し分けを固定する:
 *   - 講師 × 家モード（教室端末マーク無し）× 教室限定パス → 全画面ブロック
 *   - 講師 × 家モード × 家OKパス（/today 等） → 何も出さない
 *   - 講師 × 教室端末 → 何も出さない
...
// homeMode の算出は本物（lib/homeMode.ts）を通す。判定式のズレも一緒に検知したいため。
vi.mock('@/contexts/DeviceTrustContext', () => ({
  useDeviceTrust: () => ({
    isTrustedDevice: state.trusted,
    loading: false,
    homeMode: isHomeModeRestricted(state.role, state.trusted),
    refresh: vi.fn(),
  }),
}));
...
describe('HomeModeGate', () => {
  it('講師×家モード×教室限定パスはブロック表示になる', () => {
    render(<HomeModeGate />);
    expect(screen.getByText(BLOCK_TEXT)).toBeTruthy();
  });

  it('ブロック時は家OKページへの導線とログアウトを出す（閉じ込め防止）', () => {
    render(<HomeModeGate />);
  ...
  it.each([
    ...
    '/help',
    '/settings/account',
  ])('家OKパス（%s）では何も出さない', (path) => {
    state.pathname = path;
    const { container } = render(<HomeModeGate />);
