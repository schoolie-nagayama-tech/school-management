...
const PUBLIC_PATHS = [
  ...
  // 各ページのセッション確認＋RLSで行う。
  '/mypage',
  // 法務文書の公開ページ（P3-L3）。登録前の保護者・入塾検討者が読むため未ログインで見せる。
  // ★ middleware の isPublicRoute に足すだけでは不足する:
  //   middleware はサーバー側のセッション更新をスキップするだけで、ハイドレーション後に
  //   このクライアント側ガードが /login へ飛ばしてしまう。両方に足して初めて公開される。
  '/privacy',
  '/terms',
];
