...
import { isHomeModeRestricted } from '@/lib/homeMode';
 ...
 *
 * ★ 問い合わせるのは講師のときだけ:
 *   manager 以上は家でもフル（§1-3）なので判定自体が不要。無駄なリクエストを出さない。
 *
 * ★ 判定確定前は制限をかけない:
 *   未確定（null）のうちに家モード扱いすると、教室PCの講師にも一瞬ブロック画面が
 *   出てしまう。homeMode は確定後にだけ true になる。
 */
...
interface DeviceTrustContextType {
  ...
  loading: boolean;
  /** 家モードか。isHomeModeRestricted の結果を確定後にだけ返す */
  homeMode: boolean;
  /** 端末登録・失効の直後に判定を取り直す */
...
export function DeviceTrustProvider({ children }: { children: ReactNode }) {
  ...
  const load = useCallback(
    async (force: boolean) => {
      ...
      try {
        ...
        // 判定不能は「信頼しない」側に倒す（= 家モード）。サーバー側と同じ方針。
        cachedTrusted = false;
      }
      setTrusted(cachedTrusted);
    },
  ...
  return (
    <DeviceTrustContext.Provider
      value={{
        ...
        homeMode: trusted !== null && isHomeModeRestricted(profile?.role, trusted),
        refresh,
      }}
    >
      {children}
...
export function useDeviceTrust(): DeviceTrustContextType {
  const ctx = useContext(DeviceTrustContext);
  if (ctx === undefined) {
