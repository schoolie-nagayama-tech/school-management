...
import type { NotificationInitialData } from '@/lib/api/notifications';
import {
  generateStudentCSV,
...
export function StudentsPageClient({
  bulletinSlot,
  // アラートボードの SSR 事前取得スロット（page.tsx で Suspense ストリーミング）。
  alertSlot,
  // 通知フィードの SSR 事前取得データ（page.tsx のサーバー側で解決して渡る）。
  // onStudentClick を保持したままクライアントで描画するため、slot ではなくデータ prop で受ける。
  notificationInitialData,
}: {
  bulletinSlot?: ReactNode;
  alertSlot?: ReactNode;
  notificationInitialData?: NotificationInitialData;
}) {
  // 権限チェック
