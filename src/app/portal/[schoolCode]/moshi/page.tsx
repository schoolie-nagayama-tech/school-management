...
import { getActiveMoshiPeriod } from '@/lib/api/moshi';
...
export default function MoshiPortalPage() {
  ...
  const [period, setPeriod] = useState<MoshiPeriod | null>(null);
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        // 公開中の期間を取得
        const periodData = await getActiveMoshiPeriod(schoolCode);
        if (!periodData) {
    ...
    <div className="min-h-screen bg-[#f8fafc]">
      <div className="max-w-lg mx-auto px-4 py-8 w-full">
        {!period ? (
          ...
          // フォーム表示
          <MoshiForm school={school} period={period} />
        )}
      </div>
    </div>
  );
