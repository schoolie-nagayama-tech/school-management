...
import { getActiveSoudanPeriod } from '@/lib/api/soudan';
...
export default function SoudanPortalPage() {
  ...
  const [period, setPeriod] = useState<SoudanPeriod | null>(null);
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        // 公開中の期間を取得
        const periodData = await getActiveSoudanPeriod(schoolCode);
        if (!periodData) {
      ...
      <div className="max-w-lg mx-auto px-4 py-8 w-full">
        {!period ? (
          ...
          <div className="bg-white rounded-xl border border-[#e5e7eb] p-8 text-center">
            <h1 className="text-2xl font-bold text-[#1f2937] mb-4">お客様相談</h1>
            <p className="text-[#4b5563] mb-6">現在、お客様相談の受付は行っておりません。</p>
            <a
              href={`/portal/${schoolCode}`}
              className="inline-block px-6 py-3 bg-[#3b82f6] text-white font-medium rounded-lg hover:bg-[#60a5fa] transition-colors duration-150"
            >
          ...
          // フォーム表示
          <SoudanForm school={school} period={period} />
        )}
      </div>
    </div>
  );
