...
// ─── デモ用：報告書の下に並べる進行表イメージ（ダミー）─────────
// 教材×単元のテーブルと、直近の授業記録フィードの2部構成。
// 機能検討用のイメージで、実データとは連動しない。
function DemoProgressPreview() {
  // 教材×単元テーブルのダミー行
  const units = [
  ...
  return (
    <div className="mt-6 border-t-2 border-dashed border-info/30 pt-4">
      <div className="rounded-lg bg-info-subtle border border-info/30 px-3 py-2 text-xs text-info mb-3">
        ...
        実際は「進行表を見ながら上の報告書を書く」動線を想定し、同じページに並べています。
      </div>

      {/* 教材×単元テーブル */}
      <div className="mb-5">
        <h3 className="text-sm font-bold text-text-heading mb-2">
          進行表：New Horizon 中2 英語
        </h3>
      ...
      {/* 直近の授業記録フィード */}
      <div>
        <h3 className="text-sm font-bold text-text-heading mb-2">直近の授業記録</h3>
              ...
              <li key={i} className="rounded-lg border border-border-subtle p-2.5 bg-white">
                <div className="flex items-center gap-2 text-xs mb-1">
                  <span className="font-semibold text-text-body tabular-nums">
                    {f.date.slice(5).replace('-', '/')}（{W2[d.getDay()]}）
                  </span>
