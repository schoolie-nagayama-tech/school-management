...
import { getLoginLinks, saveLoginLinks, type LoginLink } from '@/lib/api/login-links';
...
export default function LoginLinksSettingsPage() {
  // 教室長以上のみアクセス可
  ...
  // 取得は未認証でも可能だが、権限判定を待ってから読み込む
  useEffect(() => {
    if (!profile?.id) return;
    let cancelled = false;
    void (async () => {
      const data = await getLoginLinks();
      if (!cancelled) {
        setLinks(data);
        setIsLoading(false);
      }
  ...
  // 行を更新
  const handleChange = useCallback((id: string, patch: Partial<LoginLink>) => {
  ...
  const handleSave = useCallback(async () => {
    ...
    if (trimmed.length > 6) {
      toastError('リンクは6件まで登録できます');
      return;
    }

    setIsSaving(true);
    try {
      const saved = await saveLoginLinks(trimmed);
      setLinks(saved);
      setDirty(false);
      success('ログインリンクを保存しました');
    } catch (e) {
  ...
  if (permissionLoading) {
    return (
      <AdminLayout headerTitle="ログイン画面リンク設定">
      ...
      <div>
        ...
        <div className="mb-6">
          ...
          <p className="text-sm text-text-muted">
            ログイン画面の下部（「パスワードをお忘れですか？」の近く）に表示する、
            <strong>ログインせずにアクセスできる業務用サイト</strong>へのリンクを管理します。
            ここで設定した内容は<strong>全教室共通</strong>で、
            <strong>ログインしていない状態でも表示</strong>されます。
            機密情報を含まない公開URLのみを登録してください（想定は2件程度）。
          </p>
        </div>
        ...
        <div className="mt-4 flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3">
          <Button variant="outline" onClick={handleAdd} disabled={isSaving || links.length >= 6}>
        ...
        {links.some((l) => l.label.trim() && /^https?:\/\//i.test(l.url.trim())) && (
          <div className="mt-8">
            ...
            <div className="flex flex-wrap items-center justify-center gap-x-3 gap-y-1.5 px-3 py-4 rounded-xl border border-border bg-surface-raised">
              {links
