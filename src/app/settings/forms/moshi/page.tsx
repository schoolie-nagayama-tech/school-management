...
import {
  getMoshiPeriods,
  deleteMoshiPeriod,
  getMoshiResponseCount,
  archiveMoshiPeriod,
  unarchiveMoshiPeriod,
} from '@/lib/api/moshi';
...
export default function MoshiSettingsPage() {
  ...
  const handleArchivePeriod = async (period: MoshiPeriod) => {
    ...
    try {
      setIsSubmitting(true);
      const schoolId = getDefaultSchoolId();
      const result = await archiveMoshiPeriod(period.id, schoolId, period.period_key);
      await fetchPeriods();
  ...
  const handleUnarchivePeriod = async (period: MoshiPeriod) => {
    try {
      setIsSubmitting(true);
      const schoolId = getDefaultSchoolId();
      const result = await unarchiveMoshiPeriod(period.id, schoolId, period.period_key);
      await fetchPeriods();
  ...
  return (
    <div>
      ...
      <AdminLayout headerTitle="オープン模試申し込み 設定">
        <div className="mb-4">
          <Link
            href="/settings"
              ...
              <table className="w-full border-collapse border border-border text-sm">
                <thead>
                  <tr className="bg-surface-hover">
                    ...
                    <th className="border border-border px-4 py-3 text-left">受験日時</th>
                ...
                <tbody>
                  ...
                  {showArchived && archivedPeriods.length > 0 && (
                    <>
                      <tr>
                        <td colSpan={7} className="border border-border px-4 py-2 bg-gray-50">
