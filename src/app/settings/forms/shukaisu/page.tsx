...
export default function ShukaisuSettingsPage() {
  const [periods, setPeriods] = useState<ShukaisuPeriod[]>([]);
  const [responseCounts, setResponseCounts] = useState<Record<string, number>>({});
  const [isLoading, setIsLoading] = useState(true);
  const [errorMessage, setErrorMessage] = useState('');
  const [isEditorOpen, setIsEditorOpen] = useState(false);
  const [editingPeriod, setEditingPeriod] = useState<ShukaisuPeriod | null>(null);
  ...
  const fetchPeriods = useCallback(async () => {
    ...
    try {
      ...
      const data = await getShukaisuPeriods(schoolId, showArchived);
      setPeriods(data);
      ...
      await Promise.all(
        data.map(async (period) => {
          try {
            const count = await getShukaisuResponseCount(schoolId, period.period_key);
  ...
  // 期間ステータスの取得
  const getPeriodStatus = (period: ShukaisuPeriod) => {
  ...
  // 期間アーカイブ解除
  const handleUnarchivePeriod = async (period: ShukaisuPeriod) => {
    try {
      setIsSubmitting(true);
      const schoolId = getDefaultSchoolId();
                        ...
                        <div className="flex gap-2">
                          ...
                          <Button
                            onClick={() => {
                              const schoolId = getDefaultSchoolId();
                              window.open(
                                `/forms/preview-period/shukaisu/${period.period_key}?schoolId=${schoolId}`,
                                '_blank'
                              );
                            }}
                            variant="secondary"
                          ...
                          <Link
                            href={`/forms/responses/shukaisu/${period.period_key}`}
