...
import {
  getYoubiPeriods,
  deleteYoubiPeriod,
  getYoubiResponseCount,
  archiveYoubiPeriod,
  unarchiveYoubiPeriod,
} from '@/lib/api/youbi';
...
export default function YoubiSettingsPage() {
  ...
  const handleArchivePeriod = async (period: YoubiPeriod) => {
    ...
    try {
      setIsSubmitting(true);
      const schoolId = getDefaultSchoolId();
      const result = await archiveYoubiPeriod(period.id, schoolId, period.period_key);
      await fetchPeriods();
  ...
  const handleUnarchivePeriod = async (period: YoubiPeriod) => {
    try {
      setIsSubmitting(true);
      const schoolId = getDefaultSchoolId();
      const result = await unarchiveYoubiPeriod(period.id, schoolId, period.period_key);
      await fetchPeriods();
