...
import {
  getSoudanPeriods,
  deleteSoudanPeriod,
  getSoudanResponseCount,
  archiveSoudanPeriod,
  unarchiveSoudanPeriod,
} from '@/lib/api/soudan';
...
export default function SoudanSettingsPage() {
  ...
  const handleArchivePeriod = async (period: SoudanPeriod) => {
    ...
    try {
      setIsSubmitting(true);
      const schoolId = getDefaultSchoolId();
      const result = await archiveSoudanPeriod(period.id, schoolId, period.period_key);
      await fetchPeriods();
  ...
  const handleUnarchivePeriod = async (period: SoudanPeriod) => {
    try {
      setIsSubmitting(true);
      const schoolId = getDefaultSchoolId();
      const result = await unarchiveSoudanPeriod(period.id, schoolId, period.period_key);
      await fetchPeriods();
