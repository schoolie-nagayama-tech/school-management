...
import { getSoudanPeriods, deleteSoudanPeriod, getSoudanResponseCount, archiveSoudanPeriod, unarchiveSoudanPeriod } from '@/lib/api/soudan';
...
export default function SoudanSettingsPage() {
  ...
  const [editingPeriod, setEditingPeriod] = useState<SoudanPeriod | null>(null);
  ...
  const fetchPeriods = useCallback(async () => {
    ...
    try {
      ...
      const data = await getSoudanPeriods(schoolId, showArchived);
      setPeriods(data);
      ...
      await Promise.all(
        data.map(async (period) => {
          try {
            const count = await getSoudanResponseCount(schoolId, period.period_key);
  ...
  // 期間ステータスの取得
  const getPeriodStatus = (period: SoudanPeriod) => {
  ...
  const handleDelete = async (periodId: string, periodTitle: string) => {
    ...
    try {
      ...
      await deleteSoudanPeriod(periodId);
      await fetchPeriods();
      success('期間を削除しました');
    } catch (err) {
  ...
  // 期間アーカイブ
  const handleArchivePeriod = async (period: SoudanPeriod) => {
    if (
    ...
    try {
      setIsSubmitting(true);
      const schoolId = getDefaultSchoolId();
      const result = await archiveSoudanPeriod(
        period.id,
        schoolId,
        period.period_key
      );
  ...
  // 期間アーカイブ解除
  const handleUnarchivePeriod = async (period: SoudanPeriod) => {
    try {
      setIsSubmitting(true);
      const schoolId = getDefaultSchoolId();
      const result = await unarchiveSoudanPeriod(
        period.id,
        schoolId,
        period.period_key
      );
                        ...
                        <div className="flex gap-2">
                          ...
                          <Button
                            onClick={() => {
                              const schoolId = getDefaultSchoolId();
                              window.open(
                                `/forms/preview-period/soudan/${period.period_key}?schoolId=${schoolId}`,
                                '_blank'
                              );
                            }}
                            variant="secondary"
                          ...
                          <Link
                            href={`/forms/responses/soudan/${period.period_key}`}
