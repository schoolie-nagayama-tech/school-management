...
/**
 * 講師の「家モード」判定（クライアント/サーバー共用の純関数）。
 *
 * 正典: docs/teacher-home-mode-plan.md §1・§2
 *
 * 判定式は1つだけ:
 *   role=teacher かつ 信頼済み端末（教室端末マーク）でない → 家モード。
 *   manager 以上は管理責任側なので家でもフル機能（家モードにならない）。
 *
 * ★ この境界を各所に散らさないこと:
 *   ページゲート（HomeModeGate）・ナビの出し分け（navConfig / MobileBottomNav）が
 ...
 *   「家から自分の予定と勤怠だけ見る」を成立させるための最小セットなので、
