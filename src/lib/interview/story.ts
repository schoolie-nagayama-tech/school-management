 ...
 * ★2026-09-27 に「テストなし」（assessment_scores.no_test）ができ、定期テスト・通知表の既存の 0 は
 *   テストなしに置き換えた（値は null）。いまは 0 を入れる理由が無いので、ここで拾うのは
 *   置き換え後に古い習慣や入れ間違いで入った 0 だけ。安全網として残す（外すと前回比の誤読が戻る）。
 */
