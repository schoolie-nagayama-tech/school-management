...
--
-- 本番(東京: bniistrbylypnwpfqszb)には 2026-07-14 に MCP 経由で適用済み。プレースホルダーではなく、
-- 既存の組み込みトリガーに入っていた service_role JWT を SQL で参照して Vault へ移設したため
-- (鍵の文字列を新規に書かず・平文→暗号化の移動)、メール送信を一瞬も止めずに移行できた。
-- 新規環境ではこのプレースホルダー登録のあと、上記 vault.update_secret で実キーを設定する。
